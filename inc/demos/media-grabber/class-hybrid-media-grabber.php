...
class Hybrid_Media_Grabber {
	...
	public function do_block_media() {
		...
		if ( function_exists( 'gutenberg_parse_blocks' ) ) {
			$function_name = 'gutenberg_parse_blocks';
		}

		//WP 5.0
		if ( function_exists( 'parse_blocks' ) ) {
			$function_name = 'parse_blocks';
		}

		if ( empty( $function_name ) ) {
			return; //not gutenberg
		}

		$blocks = call_user_func( $function_name, $this->content );

		if ( !empty( $blocks ) ) {
			foreach ( $blocks as $block ) {
				if ( $block['blockName'] == 'core/'.$this->type ) {
					$this->media = $block['innerHTML'];
